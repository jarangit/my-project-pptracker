...
const TopRating = ({ data }: Props) => {
  return (
    <>
      {/* FootAPI */}
      <Text value="TOP RATING" size="xl" className='font-bold text-center mb-3' />
      <Column gap='2' className='justify-between'>
        {data?.slice(0, 10).map((item: any, key: any) => (
          <div key={key}>
            <PlayerScore data={item} index={key} score={(item.statistics.rating).toFixed(1)} />
          </div>
        ))}
      </Column>
    </>
